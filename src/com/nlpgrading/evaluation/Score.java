package com.nlpgrading.evaluation;
...
public class Score {
	...
	/**
	 * compute the final weighted score based on the syntactic, semantic and
	 * essay length parameters and their respective weights
	 * 
	...
	public float getFinalScore() {
		...
		int weightedSyntaxScore = this.getWordOrderScore()
				+ this.getSubjectVerbAgreementScore()
				+ this.getVerbUsageScore()
				+ (2 * this.getSentenceFormationScore());

		int weightedSemanticsScore = this.getCoherenceScore()
				+ (3 * this.getTopicAdherenceScore());

		finalScore = (weightedSyntaxScore + weightedSemanticsScore + this
				.getEssayLengthScore()) / 10.0f;
