...
public class AutoGrader {
	...
	public ArrayList<PosTag> getStanfordPosTags(String text) {
		...
		//	System.out.println(tagged);
			...
			e.printStackTrace();
			System.out
					.println("[Error] Stanford POS tagger model loading failed");
	...
	public void gradeEssayLength(Essay essay) {
		...
		for (int i = 0; i < essay.getSentences().size(); i++) {
			...
			for (PosTag pos : tags) {
				...
				if (pos.getPartOfSpeech() == PartOfSpeech.VB
						|| pos.getPartOfSpeech() == PartOfSpeech.VBD
						|| pos.getPartOfSpeech() == PartOfSpeech.VBG
						|| pos.getPartOfSpeech() == PartOfSpeech.VBN
						|| pos.getPartOfSpeech() == PartOfSpeech.VBP
						|| pos.getPartOfSpeech() == PartOfSpeech.VBZ) {
	...
	public PartOfSpeech getPOS(String posText) {
		...
		try {
			...
			System.out
					.println("[Error] No such part of speech, using default FW");
			return returnPos;
	...
	public void segmentEssay(Essay essay) {
		...
		try {
			...
			smodel = new FileInputStream(System.getProperty("user.dir")
					+ "/Models/en-sent.bin");
			...
			for (int i = 0; i < essay.getSentences().size(); i++) {
				...
				newsent.addAll(Arrays.asList(sentenceDetector.sentDetect(essay
						.getSentences().get(i))));
			}
	...
	public void gradeSyntax(Essay essay) {
		try {
			...
			for (int i = 0; i < parse.size(); i++) {
				...
				if (allS.size() > 1) {
					...
					for (int j = 0; j < allS.size(); j++) {
						...
						if (node != null) {
							...
							if (tag != null)
								subject = new PosTag(tag.toString(),
										getPOS(tag.getType()));
						}
						...
						if (node != null) {
							...
							if (tag != null)
								mainVerb = new PosTag(tag.toString(),
										getPOS(tag.getType()));

							tag = BFS(node, nouns);

							if (tag != null)
								object = new PosTag(tag.toString(),
										getPOS(tag.getType()));
						}

						if (mainVerb.getPartOfSpeech() == PartOfSpeech.VB)
							;
						else if (PartOfSpeech.getPersonType(
								subject.getPartOfSpeech(), subject.getString()) != PartOfSpeech
								.getPersonType(mainVerb.getPartOfSpeech(),
										mainVerb.getString())) {
				...
				if (allVerbs.size() > 0) {
					...
					for (int c = 0; c < allVerbs.size(); c++) {
						...
						if (PartOfSpeech.getTenseType(getPOS(allVerbs.get(c)
								.getType())) == Tense.PAST)
							tensecounts[0] = tensecounts[0] + 1;
						else if (PartOfSpeech.getTenseType(getPOS(allVerbs.get(
								c).getType())) == Tense.PRESENT_PARTICIPLE)
							tensecounts[1] = tensecounts[1] + 1;
						else if (PartOfSpeech.getTenseType(getPOS(allVerbs.get(
								c).getType())) == Tense.PRESENT)
							tensecounts[2] = tensecounts[2] + 1;
						else if (PartOfSpeech.getTenseType(getPOS(allVerbs.get(
								c).getType())) == Tense.PAST_PARTICIPLE)
				...
				if (top.toString().contains("because")
						&& BFS(parse.get(i), PartOfSpeech.SBAR.toString()) != null)
					derrorcount = derrorcount + 1;
			}
			...
			for (int p = 0; p < essay.getPosTags().size(); p++) {
				...
				for (int q = 0; q < essay.getPosTags().get(p).size() - 1; q++) {
					...
					for (int a = 0; a < rules.size(); a++) {
						if (essay.getPosTags().get(p).get(q).getPartOfSpeech() == rules
								.get(a).getPos1()
								&& essay.getPosTags().get(p).get(q + 1)
										.getPartOfSpeech() == rules.get(a)
										.getPos2())
							aerrorcount = aerrorcount + 1;
			...
			essay.getEssayScore().setVerbUsageScore(
					essay.getEssayScore().computeScore1c(cerrorcount,
							allVerbs.size()));
			essay.getEssayScore().setWordOrderScore(
					essay.getEssayScore().computeScore1a(aerrorcount,
							essay.getLength()));
			essay.getEssayScore().setSentenceFormationScore(
					essay.getEssayScore().computeScore1d(derrorcount,
							dtotalcount));
	...
	public void gradeTopicCoherence(Essay essay) {
		...
		essay.getEssayScore().setTopicAdherenceScore(
				essay.getEssayScore().computeScore2b(b2ErrorCount,
						commonNounList.size()));
	...
	private boolean isTopicCoherent(ArrayList<String> list, String text) {
		...
		return !(list.contains(text) || !hypernymCheck(text) || !meronymCheck(text)||isKeywordPresent(text, list));
	}
	...
	private boolean hypernymCheck(String text) {
		...
		edu.smu.tspell.wordnet.Synset[] textSynsets = wn.getSynsets(text,
				SynsetType.NOUN);
	...
	private boolean meronymCheck(String text) {
		...
		allmeronyms.addAll(Arrays.asList(nounSynset.getPartMeronyms()[0]
				.getWordForms()));
	...
	public void gradeTextCoherence(Essay essay) {
		...
		for (int i = 0; i < parse.size(); i++) {
			...
			for (int j = 0; j < allPronouns.size(); j++) {
				...
				PosTag pronoun = new PosTag(allPronouns.get(j).toString(),
						getPOS(allPronouns.get(j).getType()));

				if (PartOfSpeech.getPersonType(pronoun.getPartOfSpeech(),
						pronoun.getString()) == Person.THIRD) {
					// bonus
					bonus_2a = bonus_2a + 1;
				} else if (PartOfSpeech.getPersonType(
						pronoun.getPartOfSpeech(), pronoun.getString()) == Person.SECOND) {
			...
			for (int j = 0; j < allPronouns.size(); j++) {
				...
				if (antecedents.size() > 0) {
					...
					EntityGen neweg = new EntityGen(
							candidateAntecedent.getEntity(), 0); // current
					entities.add(neweg);
				}
			}

			/*
			 * for(EntityGen eg : entities){ System.out.print(eg.getEntity() +
			 * "-" + eg.getGeneration() + " <- "); }
			 * 
			 * System.out.print("\n");
			 */
		...
		// float total = bonus_2a/(float)parse.size() -
		// errorcount_2a/(float)parse.size();
		// System.out.println(size);
		// System.out.println(bonus_2a);
		// System.out.println(errorcount_2a);
		essay.getEssayScore().setCoherenceScore(
				essay.getEssayScore().computeScore2a(errorcount_2a,
						size + bonus_2a));

	}

	private Stack<EntityGen> hasAntecedent(Parse parse,
		...
		for (EntityGen eg : entities) {
			...
			if (PartOfSpeech.getGenderType(parse.toString()) == PartOfSpeech
					.getGenderType(eg.getEntity())
					&& PartOfSpeech.getNumberType(getPOS(parse.getType()), "") == PartOfSpeech
							.getNumberType(null, eg.getEntity())) {

				en.add(eg);
			}

			if (PartOfSpeech.getNumberType(getPOS(parse.getType()), "") != PartOfSpeech
